...
int8_t PN532_HSU::writeCommand(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    ...
    uart_putc(uart1,PN532_PREAMBLE);
    uart_putc(uart1,PN532_STARTCODE1);
    uart_putc(uart1,PN532_STARTCODE2);

    uint8_t length = hlen + blen + 1;   // length of data field: TFI + DATA
    uart_putc(uart1,length);
    uart_putc(uart1,~length + 1);         // checksum of length

    uart_putc(uart1,PN532_HOSTTOPN532);
    ...
    for (uint8_t i = 0; i < hlen; i++) {
        sum += header[i];
        uart_putc(uart1, header[i]);
        DMSG_HEX(header[i]);
    }

    for (uint8_t i = 0; i < blen; i++) {
        sum += body[i];
        uart_putc(uart1, body[i]);
        DMSG_HEX(body[i]);
    }

    uint8_t checksum = ~sum + 1;            // checksum of TFI + DATA
    uart_putc(uart1,checksum);
    uart_putc(uart1,PN532_POSTAMBLE);
